...
class UserInterface(DirectObject.DirectObject):
  def __init__(self,whichPlayer,Gorillas,buildingList,theScores,mySun,myNode,AL,p1Name,p2Name):
    ...
    self.arrowHprInterval1 = self.arrow.hprInterval(.5,Vec3(360,0,0),startHpr = Vec3(0,0,0))
    self.arrowHprInterval2 = self.arrow.hprInterval(.5,Vec3(0,0,0),startHpr = Vec3(360,0,0))
    ...
    self.textNodePath.setColor(1,1,1,1)
    ...
    ## self.player1Label(self.playerOneName)
    ## self.player1Label(self.playerTwoName)
    self.player1Label(self.playerOneName)
    self.player2Label(self.playerTwoName)
...
##      self.playerTwo.removeAll()
  ...
  def player1Label(self, p1Name):
    self.p1Label = OnscreenText(text=p1Name + ": " + str(self.scores[0]),
                                pos=(-1.2,.92), scale = .07)
    self.p1Label.setColor(0,0,1,1)

  def player2Label(self, p2Name):
    self.p2Label = OnscreenText(text=str(self.scores[1]) + "  :" + p2Name,
                                pos=(1.2,.92), scale = .07)
    self.p2Label.setColor(1,0,0,1)
